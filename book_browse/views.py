...
def index(request):
    ...
    return render(request, 'index.html', context={
        'form': form, 'num_books': num_books, 'num_instances': num_instances,
...
def books(request):
    ...
    if (search is False and author is False) or (
                                                search == "" and author == ""):
        return redirect('/')
    ...
    if r.status_code != 200:
        return render(request, 'book_browse/books.html', {
            'message': 'Извините, похоже,\
             сейчас возникла проблема с Google Книгами.'})

    data = r.json()

    if 'items' not in data:
        return render(request, 'book_browse/books.html', {'message': '\
        Извините, ни одна книга не соответствует этому поисковому запросу.'})
    ...
    for book in fetched_books:
        book_dict = {
            ...
            'image': book['volumeInfo']['imageLinks']['thumbnail'] if
            'imageLinks' in book['volumeInfo'] else "Изображение не найдено",
            'authors': ", ".join(book['volumeInfo']['authors']) if
            'authors' in book['volumeInfo'] else "Неизвестен",
            'publisher': book['volumeInfo']['publisher'] if
            'publisher' in book['volumeInfo'] else "Неизвестно",
            'info': book['volumeInfo']['infoLink'],
            'popularity': book['volumeInfo']['ratingsCount'] if
            'ratingsCount' in book['volumeInfo'] else 0
        }
...
class AuthorDetailView(generic.DetailView):
    ...
    def author_detail_view(request, primary_key):
        ...
        return render(request, 'book_browse/author_detail.html',
...
class LoanedBooksByUserListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        return BookInstance.objects.filter(
            borrower=self.request.user).order_by('due_back')
